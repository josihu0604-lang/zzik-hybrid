  ...
  return (
    <Headless.MenuItems
      ...
      className={clsx(
        ...
        // Base styles (DES-171: 개선된 스타일)
        'isolate w-max rounded-xl p-1',
        // Invisible border that is only visible in `forced-colors` mode for accessibility purposes
        'outline outline-transparent focus:outline-hidden',
        // Handle scrolling when menu won't fit in viewport
        'overflow-y-auto max-h-[min(var(--available-height,400px),400px)]',
        // Popover background (DES-171: ZZIK 브랜드 적용)
        'bg-[rgba(18,19,20,0.98)] backdrop-blur-xl dark:bg-[rgba(18,19,20,0.98)]',
        // Shadows (DES-171: 개선된 깊이감)
        'shadow-lg ring-1 ring-white/10 dark:ring-white/10 dark:ring-inset',
        // Define grid at the menu level if subgrid is supported
        'supports-[grid-template-columns:subgrid]:grid supports-[grid-template-columns:subgrid]:grid-cols-[auto_1fr_1.5rem_0.5rem_auto]',
        // Transitions (DES-171: 부드러운 애니메이션)
        'transition-all duration-200 ease-out',
        'data-leave:duration-150 data-leave:ease-in',
        'data-closed:data-leave:opacity-0 data-closed:data-leave:scale-95',
        // DES-171: 액세서빌리티 개선
...
export function DropdownItem({
  ...
  let classes = clsx(
    className,
    // Base styles (mobile py-3 for 44px touch target)
    'group cursor-default rounded-lg px-3.5 py-3 focus:outline-hidden sm:px-3 sm:py-1.5',
    // Text styles
