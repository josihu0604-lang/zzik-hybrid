...
export default function LeaderError({ error, reset }: ErrorProps) {
  useEffect(() => {
    if (process.env.NODE_ENV === 'production') {
      Sentry.captureException(error, {
        tags: { component: 'LeaderErrorBoundary', page: 'leader' },
        level: 'error',
      });
    }
  }, [error]);
  ...
  return (
    <div className="min-h-screen bg-space-950 flex items-center justify-center p-4">
      ...
      >
        ...
        >
          <TrendingUp className="w-8 h-8 text-spark-500" />
        </div>

        <h1 className="text-white text-xl font-bold mb-3">리더 대시보드를 불러올 수 없습니다</h1>
